...
def DowbloadAIMSData(ReportType):
    ...
    if ReportType == "NonFood":
        ...
        con = cx_Oracle.connect(user="paragj", password="par18ja24", dsn = dsnStr )    
        cur = con.cursor() 
        with open(Script_Path,'r') as script:
            query = script.read()
            cur.execute(query)
...
rows = pd.read_excel(Path + r'\AIMS_Invoice_Reco.xlsx', sheet_name = "AIMS Data")
...
aims_row_df = df
#filter AIMS data for status = confirmed for calculation of article amount
...
aims_row_df = aims_row_df.merge(map_df, how = "left",left_on = "Entity Code", right_on = "Entity")
aims_row_df['VAT VAlidation'] = np.where((aims_row_df['Entity Code'] == aims_row_df['Entity']), ' Matching','Not Matching')
...
AimsDF = df.loc[:,['Entity Code','Entity Name','Customer Name','Invoice No','Invoice Date','Article Amount','VAT Collected','Invoice Amount']]


df3 = AimsDF.groupby(['Entity Code','Entity Name','Customer Name','Invoice No','Invoice Date','Invoice Amount'])['Article Amount','VAT Collected'].sum()
#dff = df3.to_frame()
#dff.reset_index(inplace = True)
df3.reset_index(inplace = True)


df4 = df3.merge(OracleDF, how = "left",left_on = "Invoice No", right_on = "Trx Number")


df4['Total Amount'] = df4['Article Amount'] + df4['VAT Collected']
df4['Variance'] = df4['Total Amount'].round() - df4['Amount'].round()
...
Reco = df4.loc[:,['Invoice No','Entity Code','Entity Name','Customer Name','Article Amount','VAT Collected','Total Amount','Invoice Amount','Amount','Variance','status']]
Reco.columns = ['Invoice No','Entity Code','Entity Name','Customer Name','Article Amount','VAT Collected','Total Amount','Invoice Amount','Oracle Amount','Variance','status']
...
with pd.ExcelWriter(output_file) as writer:
    ...
    aims_row_df.to_excel(writer, sheet_name="AIMS Data", index=False)
